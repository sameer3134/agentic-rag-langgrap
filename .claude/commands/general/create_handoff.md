...
Use the following information to understand how to create your document:
    - create your file under `{{PATH_THOUGHTS}}handoffs/{{TICKET_ID}}/YYYY-MM-DD_HH-MM-SS_ENG-ZZZZ_description.md`, where:
        - YYYY-MM-DD is today's date
        - HH-MM-SS is the hours, minutes and seconds based on the current time, in 24-hour format (i.e. use `13:00` for `1:00 pm`)
        - {{TICKET_ID}} is the ticket number (replace with `general` if no ticket)
...
# Handoff: {{TICKET_ID}} {very concise description}
...
### 3. Approve and Sync
Run `{{SYNC_COMMAND}}` to save the document.
...
```bash
/resume_handoff {{PATH_THOUGHTS}}handoffs/ENG-2166/2025-01-08_13-44-55_ENG-2166_create-context-compaction.md
```
</example_response>
