---
description: Manage {{TICKET_TOOL}} tickets - create, update, comment, and follow workflow patterns
model: sonnet
---
...
# {{TICKET_TOOL}} - Ticket Management

You are tasked with managing {{TICKET_TOOL}} tickets, including creating tickets from thoughts documents, updating existing tickets, and following the team's specific workflow patterns.

## Initial Setup

First, verify that {{TICKET_TOOL}} MCP tools are available by checking if any `mcp__linear__` tools exist. If not, respond:
```
I need access to {{TICKET_TOOL}} tools to help with ticket management. Please run the `/mcp` command to enable the {{TICKET_TOOL}} MCP server, then try again.
```
...
```
I can help you with {{TICKET_TOOL}} tickets. What would you like to do?
...
```
I'll help you create a {{TICKET_TOOL}} ticket from your thoughts document. Please provide:
...
- `{{PATH_THOUGHTS}}shared/...` → `https://github.com/{{PROJECT_NAME}}/thoughts/blob/main/repos/{{PROJECT_NAME}}/shared/...`
- `{{PATH_THOUGHTS}}{{USER_NAME}}/...` → `https://github.com/{{PROJECT_NAME}}/thoughts/blob/main/repos/{{PROJECT_NAME}}/{{USER_NAME}}/...`
- `{{PATH_THOUGHTS}}global/...` → `https://github.com/{{PROJECT_NAME}}/thoughts/blob/main/global/...`
...
- **{{BE_MODULE}}**: For tickets about the `{{PATH_DAEMON}}` directory (the daemon)
- **{{FE_MODULE}}**: For tickets about `{{PATH_WUI}}`
- **meta**: For tickets about `hlyr` commands, thoughts tool, or `{{PATH_THOUGHTS}}` directory

Note: meta is mutually exclusive with {{BE_MODULE}}/{{FE_MODULE}}. Tickets can have both {{BE_MODULE}} and {{FE_MODULE}}, but not meta with either.
...
1. **Locate and read the thoughts document:**
   ...
   - If given a topic/keyword, search {{PATH_THOUGHTS}} directory using Grep to find relevant documents
...
3. **Check for related context (if mentioned in doc):**
   ...
   - Look for any existing {{TICKET_TOOL}} tickets mentioned

4. **Get {{TICKET_TOOL}} workspace context:**
...
5. **Draft the ticket summary:**
   ...
   **Description**:
   ...
   ## References
   - Source: `{{PATH_THOUGHTS}}[path/to/document.md]` ([View on GitHub](converted GitHub URL))
...
7. **Create the {{TICKET_TOOL}} ticket:**
   ```
...
3. **File reference formatting:**
   - Wrap paths in backticks: `{{PATH_TICKETS}}example.md`
   - Add GitHub link after: `([View](url))`
   - Do this for both {{PATH_THOUGHTS}} and code files mentioned
...
4. **Comment structure example:**
   ...
   Files updated:
   - `{{PATH_DAEMON}}webhooks/handler.go` ([GitHub](link))
   - `{{PATH_THOUGHTS}}shared/rate_limit_analysis.md` ([GitHub](link))
   ```
...
3. **Present results:**
   ...
   - Include direct links to {{TICKET_TOOL}}
...
- Use proper {{TICKET_TOOL}} markdown formatting
- Include code references as: `path/to/file.ext:linenum`
- Ask for clarification rather than guessing project/status
- Remember that {{TICKET_TOOL}} descriptions support full markdown including code blocks
...
- **{{BE_MODULE}}**: `d28453c8-e53e-4a06-bea9-b5bbfad5f88a`
- **meta**: `7a5abaae-f343-4f52-98b0-7987048b0cfa`
- **{{FE_MODULE}}**: `996deb94-ba0f-4375-8b01-913e81477c4b`
...
## {{TICKET_TOOL}} User IDs

- {{USER_NAME}}: b157f9e4-8faf-4e7e-a598-dae6dec8a584
