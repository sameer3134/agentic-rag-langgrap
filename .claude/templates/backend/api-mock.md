...
from fastapi import {{BE_FRAMEWORK}}, Request, Response
import uvicorn

class MockAPIServer:
    def __init__(self, config: Dict[str, Any]):
        self.app = {{BE_FRAMEWORK}}(title="Mock API Server")
        self.routes = {}
        self.middleware = []
...
# Usage example
dynamic_stub = DynamicStub('/{{BE_PATH_API}}/users/{user_id}')
        ...
        return {
            'shopping_cart': {
                ...
                'stubs': [
                    {
                        'method': 'POST',
                        'path': '/{{BE_PATH_API}}/cart/items',
                        'handler': 'add_to_cart',
                        'modifies_state': True
                    },
                    {
                        'method': 'GET',
                        'path': '/{{BE_PATH_API}}/cart',
                        'handler': 'get_cart',
                        'uses_state': True
                    }
                ],
...
class TestingFrameworkIntegration:
    def create_jest_integration(self):
        """{{BE_TEST_FRAMEWORK}} testing integration"""
        return '''
// jest.mock.config.js
...
beforeAll(async () => {
    ...
    // Load mock definitions
    await mockServer.loadMocks('./{{BE_PATH_MOCKS}}/*.json');
...
describe('User API', () => {
    it('should fetch user details', async () => {
        ...
        // Make request
        const response = await fetch('http://localhost:3001/{{BE_PATH_API}}/users/123');
        ...
        const requests = await verifyRequests({ path: '/{{BE_PATH_API}}/users/123' });
...
@{{BE_TEST_FRAMEWORK}}.fixture(autouse=True)
...
async def test_user_creation(mock_server):
    ...
    mock.when('POST', '/{{BE_PATH_API}}/users') \
    ...
    <script>
        window.onload = function() {
            const ui = SwaggerUIBundle({
                url: "/{{BE_PATH_API}}/mock/openapi.json",
                dom_id: '#swagger-ui',
                presets: [
