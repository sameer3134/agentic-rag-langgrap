...
complexity: advanced
priority: critical
tags: [api, scaffold, backend, rest]
depends_on: []
chains_to: [backend/test-harness, backend/doc-generate]
skip_if: []
version: 1.0.0
---
...
def select_framework(requirements):
    ...
    frameworks = {
        'fastapi': {
            ...
            'example_stack': '{{BE_FRAMEWORK}} + Pydantic + SQLAlchemy + {{DB_TYPE}}'
        },
        'django_rest': {
            ...
            'example_stack': '{{BE_FRAMEWORK}} + DRF + {{DB_TYPE}} + {{DB_TYPE}}'
        },
        'express': {
            ...
            'example_stack': '{{BE_FRAMEWORK}} + {{BE_LANGUAGE}} + Prisma + {{DB_TYPE}}'
        },
        'spring_boot': {
            ...
            'example_stack': '{{BE_FRAMEWORK}} + JPA + {{DB_TYPE}} + {{DB_TYPE}}'
        }
    }
...
### 2. {{BE_FRAMEWORK}} Implementation

Complete {{BE_FRAMEWORK}} API implementation:
...
**Core Configuration**
```python
# {{BE_PATH_SRC}}/core/config.py
from pydantic import BaseSettings, validator
from typing import Optional, Dict, Any
import secrets

class Settings(BaseSettings):
    API_V1_STR: str = "/{{BE_PATH_API}}/v1"
...
def get_redis():
    """Dependency to get {{DB_TYPE}} connection"""
    return redis_client
```

**Security Implementation**
```python
# {{BE_PATH_SRC}}/core/security.py
...
**Pydantic Schemas**
```python
# {{BE_PATH_SRC}}/{{BE_PATH_SCHEMAS}}/user.py
...
# {{BE_PATH_SRC}}/{{BE_PATH_SCHEMAS}}/item.py
...
**Service Layer**
```python
# {{BE_PATH_SRC}}/services/user_service.py
...
```python
# {{BE_PATH_SRC}}/core/rate_limiting.py
import time
...
class RateLimiter:
    def __init__(self, redis_client: redis.{{DB_TYPE}}):
...
**API Endpoints**
```python
# {{BE_PATH_SRC}}/{{BE_PATH_API}}/v1/endpoints/users.py
...
```python
# {{BE_PATH_SRC}}/main.py
from fastapi import {{BE_FRAMEWORK}}, Request
...
# Create {{BE_FRAMEWORK}} app
app = {{BE_FRAMEWORK}}(
    title="Production API",
    description="A production-ready API with {{BE_FRAMEWORK}}",
    version="1.0.0",
...
### 3. {{BE_FRAMEWORK}}.js Implementation

Complete {{BE_FRAMEWORK}}.js {{BE_LANGUAGE}} implementation:
...
// package.json
{
  ...
  "version": "1.0.0",
  "scripts": {
    "dev": "nodemon {{BE_PATH_SRC}}/index.ts",
    "build": "tsc",
...
// {{BE_PATH_SRC}}/{{BE_PATH_CONFIG}}/index.ts
...
// {{BE_PATH_SRC}}/middleware/validation.ts
...
// {{BE_PATH_SRC}}/middleware/auth.ts
import { Request, Response, NextFunction } from 'express';
...
// {{BE_PATH_SRC}}/services/userService.ts
...
// {{BE_PATH_SRC}}/controllers/userController.ts
...
**{{BE_FRAMEWORK}} Tests**
```python
# {{BE_PATH_TESTS}}/conftest.py
import {{BE_TEST_FRAMEWORK}}
...
@{{BE_TEST_FRAMEWORK}}.fixture(scope="session")
def db_engine():
    Base.metadata.create_all(bind=engine)
    yield engine
    Base.metadata.drop_all(bind=engine)

@{{BE_TEST_FRAMEWORK}}.fixture(scope="function")
...
@{{BE_TEST_FRAMEWORK}}.fixture(scope="module")
def client():
    with TestClient(app) as test_client:
        yield test_client

# {{BE_PATH_TESTS}}/test_users.py
import {{BE_TEST_FRAMEWORK}}
from fastapi.testclient import TestClient
...
def test_create_user(client: TestClient):
    ...
    response = client.post("/{{BE_PATH_API}}/v1/users/", json=user_data)
...
def test_create_user_duplicate_email(client: TestClient):
    ...
    # Create first user
    response1 = client.post("/{{BE_PATH_API}}/v1/users/", json=user_data)
    assert response1.status_code == 201
    
    # Try to create second user with same email
    user_data["username"] = "user2"
    response2 = client.post("/{{BE_PATH_API}}/v1/users/", json=user_data)
    assert response2.status_code == 400
    assert "already registered" in response2.json()["detail"]

def test_get_user_unauthorized(client: TestClient):
    """Test accessing protected endpoint without token"""
    response = client.get("/{{BE_PATH_API}}/v1/users/me")
    assert response.status_code == 401

@{{BE_TEST_FRAMEWORK}}.mark.asyncio
async def test_user_authentication_flow(client: TestClient):
    ...
    create_response = client.post("/{{BE_PATH_API}}/v1/users/", json=user_data)
    ...
    login_response = client.post("/{{BE_PATH_API}}/v1/auth/login", json=login_data)
    ...
    profile_response = client.get("/{{BE_PATH_API}}/v1/users/me", headers=headers)
...
**{{CLOUD_CONTAINER_TOOL}} Configuration**
```dockerfile
# Dockerfile ({{BE_FRAMEWORK}})
...
# Install {{BE_LANGUAGE}} dependencies
...
services:
  ...
  db:
    ...
    volumes:
      ...
      - ./claude-code-bootstrap.sql:/docker-entrypoint-initdb.d/claude-code-bootstrap.sql
    ports:
      - "5432:5432"
    restart: unless-stopped
...
jobs:
  test:
    ...
    - name: Run tests
      ...
      run: |
        {{BE_TEST_FRAMEWORK}} {{BE_PATH_TESTS}}/ -v --cov=app --cov-report=xml
...
This command integrates seamlessly with other {{AI_MODEL}} Code commands to create complete development workflows:
...
/api-scaffold "User management API with {{BE_FRAMEWORK}}, {{DB_TYPE}}, and JWT auth"

# 2. Set up comprehensive testing
/test-harness "{{BE_FRAMEWORK}} API with unit, integration, and load testing using {{BE_TEST_FRAMEWORK}} and locust"

# 3. Security validation
/security-scan "{{BE_FRAMEWORK}} application with authentication endpoints"

# 4. Container optimization
/docker-optimize "{{BE_FRAMEWORK}} application with {{DB_TYPE}} and {{DB_TYPE}} dependencies"

# 5. {{CLOUD_ORCHESTRATOR}} deployment
/k8s-manifest "{{BE_FRAMEWORK}} microservice with {{DB_TYPE}}, {{DB_TYPE}}, and ingress"

# 6. Frontend integration (if needed)
/frontend-optimize "{{FE_FRAMEWORK}} application connecting to {{BE_FRAMEWORK}} backend"
```
...
/db-migrate "{{DB_TYPE}} schema migration from legacy system to modern structure"

# 2. Generate API based on migrated schema
/api-scaffold "REST API for migrated {{DB_TYPE}} schema with auto-generated models"
...
test_config:
  ...
  endpoints_to_test:
    - POST /{{BE_PATH_API}}/v1/users/
    - POST /{{BE_PATH_API}}/v1/auth/login
    - GET /{{BE_PATH_API}}/v1/users/me
    - GET /{{BE_PATH_API}}/v1/users/{id}
```
...
security_scan:
  ...
  authentication_endpoints:
    - "/{{BE_PATH_API}}/v1/auth/login"
    - "/{{BE_PATH_API}}/v1/auth/refresh"
  protected_endpoints:
    - "/{{BE_PATH_API}}/v1/users/me"
    - "/{{BE_PATH_API}}/v1/users/{id}"
  vulnerability_tests:
...
**{{CLOUD_CONTAINER_TOOL}} Integration:**
```dockerfile
# Generated Dockerfile can be optimized with /docker-optimize
# Multi-stage build for {{BE_FRAMEWORK}} application
  ...
  api-test:
    ...
    - name: Run API tests
      run: |
        ...
        {{BE_TEST_FRAMEWORK}} {{BE_PATH_TESTS}}/ -v --cov=app
    
    # Security scan (from /security-scan)
    - name: Security scan
      run: |
        bandit -r {{BE_PATH_SRC}}/
        safety check
    
    # Build optimized container (from /docker-optimize)
    - name: Build container
      run: |
        {{CLOUD_BUILD_COMMAND}} -f Dockerfile.optimized -t api:${{ github.sha }} .
    
    # Deploy to {{CLOUD_ORCHESTRATOR}} (from /k8s-manifest)
    - name: Deploy to staging
      run: |
        {{CLOUD_DEPLOY_COMMAND}} -f k8s/staging/
...
/frontend-optimize "{{FE_FRAMEWORK}} SPA with API integration, authentication, and state management"

# 3. Integration testing
/test-harness "End-to-end testing for {{FE_FRAMEWORK}} frontend and {{BE_FRAMEWORK}} backend"
...
export class APIClient {
  ...
  async createUser(userData: CreateUserRequest): Promise<User> {
    return this.request<User>('/{{BE_PATH_API}}/v1/users/', {
      method: 'POST',
  ...
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    return this.request<AuthResponse>('/{{BE_PATH_API}}/v1/auth/login', {
      method: 'POST',
  ...
  async getCurrentUser(): Promise<User> {
    return this.request<User>('/{{BE_PATH_API}}/v1/users/me');
  }
}
```
...
# Use with {{CLOUD_ORCHESTRATOR}} monitoring
/k8s-manifest "{{CLOUD_ORCHESTRATOR}} deployment with Prometheus, Grafana, and alerting"
```
