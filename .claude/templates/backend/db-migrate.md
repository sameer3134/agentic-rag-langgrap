...
depends_on: [general/create_plan]
chains_to: [backend/test-harness]
skip_if: []
version: 1.0.0
---
...
**SQL Migrations**
```sql
-- {{BE_PATH_MIGRATIONS}}/001_add_user_preferences.up.sql
BEGIN;
...
-- {{BE_PATH_MIGRATIONS}}/001_add_user_preferences.down.sql
BEGIN;
...
**Framework Migrations ({{BE_FRAMEWORK}}/{{BE_FRAMEWORK}}/Laravel)**
```python
# {{BE_FRAMEWORK}} migration
...
# Execute rollback
psql -d $DATABASE -f "{{BE_PATH_MIGRATIONS}}/${MIGRATION_VERSION}.down.sql"
...
class DynamoDBAdapter(DatabaseAdapter):
    ...
    async def _migrate_table_data(self, table, operation):
        """Migrate data between {{DB_TYPE}} tables"""
        scan_kwargs = {
...
async def migrate_sql_to_nosql():
    """Example: Migrate from {{DB_TYPE}} to {{DB_TYPE}}"""
...
**Atlas Schema Migrations ({{DB_TYPE}})**
```javascript
// atlas-migration.js
...
// Example {{DB_TYPE}} schema migration
const migration_001 = {
...
class CDCMigrationManager:
    ...
    async def setup_cdc_pipeline(self):
        ...
        # {{BE_PIPELINE_FRAMEWORK}} consumer for CDC events
        ...
        # {{BE_PIPELINE_FRAMEWORK}} producer for processed events
    ...
    async def setup_debezium_connector(self, source_db_config):
        ...
        # Submit connector to {{BE_PIPELINE_FRAMEWORK}} Connect
        import requests
...
class EnterpriseeMigrationMonitor:
    ...
    def setup_metrics_client(self):
        """Setup Prometheus/{{CLOUD_MONITORING_TOOL}} metrics client"""
    ...
    async def setup_migration_dashboard(self):
        ...
        import requests
        response = requests.post(
            f"{self.config['grafana_url']}/{{BE_PATH_API}}/dashboards/db",
...
**{{CLOUD_PROVIDER}} Database Migration with CDK**
```typescript
// aws-db-migration.ts
...
3. **Modern Tooling Integration**: Atlas, Debezium, {{BE_MIGRATION_TOOL}}, Prisma, and cloud-native solutions
4. **Change Data Capture Pipeline**: Real-time synchronization with {{BE_PIPELINE_FRAMEWORK}} and schema registry
5. **Event Sourcing Migrations**: Event store transformations and aggregate rebuilding
6. **Cloud Infrastructure Automation**: {{CLOUD_PROVIDER}} DMS, {{CLOUD_PROVIDER}} Database Migration Service, {{CLOUD_PROVIDER}} DMS
...
COMMIT;
            ...
            'connection_pool_config': {
                'fastapi': f"""
# {{BE_FRAMEWORK}} with optimized database connections
...
async def get_db_with_migration_check():
    ...
    async with get_db() as session:
        ...
        yield session
                ...
                'express': f"""
// {{BE_FRAMEWORK}}.js with database migration awareness
...
class CompleteDatabaseWorkflow:
    ...
    async def execute_complete_workflow(self):
        ...
        # 6. {{CLOUD_ORCHESTRATOR}} Deployment Update
        k8s_deployment = await self.update_k8s_deployment()
        console.log("✅ {{CLOUD_ORCHESTRATOR}} deployment updated")
