...
complexity: advanced
priority: high
tags: [docker, cloud, infra, performance]
depends_on: []
chains_to: []
skip_if: [no_docker]
version: 1.0.0
...
# {{CLOUD_CONTAINER_TOOL}} Optimization

You are a {{CLOUD_CONTAINER_TOOL}} optimization expert specializing in creating efficient, secure, and minimal container images. Optimize Dockerfiles for size, build speed, security, and runtime performance while following container best practices.

## Context
The user needs to optimize {{CLOUD_CONTAINER_TOOL}} images and containers for production use. Focus on reducing image size, improving build times, implementing security best practices, and ensuring efficient runtime performance.
...
class SmartDockerOptimizer:
    ...
    def detect_application_type(self, project_path: str) -> str:
        ...
        if any((path / f).exists() for f in ['{{CLOUD_PATH_COMPOSE}}', '{{CLOUD_ORCHESTRATOR}}']):
    ...
    def analyze_dockerfile_comprehensively(self, dockerfile_path: str, project_path: str) -> Dict[str, Any]:
        """
        Comprehensive {{CLOUD_PATH_DOCKERFILE}} analysis with modern optimization recommendations
        """
        ...
        base_image_patterns = {
            ...
            'large_base_images': {
                'patterns': [
                    ...
                    r'FROM\s+centos',
                    r'FROM\s+{{CLOUD_BASE_IMAGE}}(?!.*slim)',
                    r'FROM\s+node(?!.*{{CLOUD_BASE_IMAGE}})'
                ],
                'severity': 'medium',
                'recommendation': 'Consider using smaller alternatives ({{CLOUD_BASE_IMAGE}}, slim, {{CLOUD_BASE_IMAGE}})'
            },
            'missing_multi_stage': {
    ...
    def _analyze_layer_efficiency(self, content: str, analysis: Dict):
        """Analyze {{CLOUD_CONTAINER_TOOL}} layer efficiency and caching opportunities"""
        lines = content.split('\n')
    ...
    def _get_security_implementation(self, app_type: str) -> str:
        ...
        implementations = {
            ...
            'microservice': '{{CLOUD_BASE_IMAGE}} base, static compilation, capability dropping',
...
**Advanced Multi-Framework {{CLOUD_PATH_DOCKERFILE}} Generator**
```python
class FrameworkOptimizedDockerfileGenerator:
    ...
    def generate_optimized_dockerfile(self, framework: str, config: Dict[str, Any]) -> str:
        """Generate highly optimized {{CLOUD_PATH_DOCKERFILE}} for specific framework"""
    ...
    def _generate_node_express_optimized(self, config: Dict) -> str:
        """Generate optimized {{LANGUAGE}} {{BE_FRAMEWORK}} {{CLOUD_PATH_DOCKERFILE}}"""
        node_version = config.get('node_version', '20')
        use_bun = config.get('use_bun', False)
        
        if use_bun:
            return f"""
# Optimized {{LANGUAGE}} with Bun - Ultra-fast builds and runtime
...
# Production stage
FROM gcr.io/{{CLOUD_BASE_IMAGE}}/nodejs{node_version}-debian11
WORKDIR /app
...
"""
        ...
        return f"""
# Optimized {{LANGUAGE}} {{BE_FRAMEWORK}} - Production-ready multi-stage build
FROM node:{node_version}-{{CLOUD_BASE_IMAGE}} AS deps
...
# Build stage
FROM node:{node_version}-{{CLOUD_BASE_IMAGE}} AS build
WORKDIR /app
COPY package*.json ./
...
# Production stage
FROM node:{node_version}-{{CLOUD_BASE_IMAGE}} AS production
...
"""
    ...
    def _generate_python_fastapi_optimized(self, config: Dict) -> str:
        ...
        if use_uv:
            return f"""
# Ultra-fast {{LANGUAGE}} with uv package manager
FROM python:{python_version}-slim AS base

# Install uv - the fastest {{LANGUAGE}} package manager
RUN pip install uv
...
"""
        ...
        # Standard optimized {{LANGUAGE}} {{CLOUD_PATH_DOCKERFILE}}
        return f"""
# Optimized {{LANGUAGE}} {{BE_FRAMEWORK}} - Production-ready
...
"""
    ...
    def _generate_golang_optimized(self, config: Dict) -> str:
        """Generate optimized Go {{CLOUD_PATH_DOCKERFILE}} with minimal final image"""
        go_version = config.get('go_version', '1.21')
        
        return f"""
# Optimized Go build - Ultra-minimal final image
FROM golang:{go_version}-{{CLOUD_BASE_IMAGE}} AS build
    ...
    def _check_base_image(self, content, analysis):
        ...
        if from_match:
            ...
            optimization_recommendations = {
                'ubuntu': {
                    'alternatives': ['ubuntu:22.04-slim', '{{CLOUD_BASE_IMAGE}}:bullseye-slim', '{{CLOUD_BASE_IMAGE}}:3.18'],
                    'savings': '400-600MB',
                    'notes': 'Consider {{CLOUD_BASE_IMAGE}} for production'
                },
                '{{CLOUD_BASE_IMAGE}}': {
                    'alternatives': ['{{CLOUD_BASE_IMAGE}}:bullseye-slim', '{{CLOUD_BASE_IMAGE}}:3.18', 'gcr.io/{{CLOUD_BASE_IMAGE}}/base'],
                    'savings': '300-500MB',
                    'notes': '{{CLOUD_BASE_IMAGE}} provides better security'
                },
                'centos': {
                    'alternatives': ['{{CLOUD_BASE_IMAGE}}:3.18', 'gcr.io/{{CLOUD_BASE_IMAGE}}/base', 'ubuntu:22.04-slim'],
                    'savings': '200-400MB',
                    'notes': 'CentOS is deprecated, migrate to alternatives'
                },
                'node': {
                    'alternatives': ['node:20-{{CLOUD_BASE_IMAGE}}', 'node:20-slim', 'gcr.io/{{CLOUD_BASE_IMAGE}}/nodejs20'],
                    'savings': '300-700MB',
                    'notes': '{{CLOUD_BASE_IMAGE}} is smallest, {{CLOUD_BASE_IMAGE}} is most secure'
                },
                'python': {
                    'alternatives': ['python:3.11-slim', 'python:3.11-{{CLOUD_BASE_IMAGE}}', 'gcr.io/{{CLOUD_BASE_IMAGE}}/python3'],
                    'savings': '400-800MB',
            ...
            for base_name, config in optimization_recommendations.items():
                if base_name in base_image and 'slim' not in base_image and '{{CLOUD_BASE_IMAGE}}' not in base_image:
            ...
            deprecated_images = {
                'centos:7': 'EOL reached, migrate to Rocky Linux or {{CLOUD_BASE_IMAGE}}',
                'ubuntu:18.04': 'LTS ended, upgrade to ubuntu:22.04',
                'node:14': 'Node 14 is EOL, upgrade to node:18 or node:20',
                'python:3.8': '{{LANGUAGE}} 3.8 will reach EOL soon, upgrade to 3.11+'
            }
...
```dockerfile
# Pattern 1: {{LANGUAGE}} with Bun - Next-generation JavaScript runtime
# 5x faster installs, 4x faster runtime, 90% smaller images
FROM oven/bun:1.0-{{CLOUD_BASE_IMAGE}} AS base
...
# Stage 4: Ultra-minimal production with {{CLOUD_BASE_IMAGE}}
FROM gcr.io/{{CLOUD_BASE_IMAGE}}/nodejs20-debian11 AS production
...
# {{CLOUD_BASE_IMAGE}} already runs as non-root
USER nonroot

# Health check using {{LANGUAGE}} built-in capabilities
...
**Advanced {{LANGUAGE}} Multi-Stage with UV Package Manager**
```dockerfile
# Pattern 2: {{LANGUAGE}} with UV - 10-100x faster than pip
FROM python:3.11-slim AS base

# Install UV - next generation {{LANGUAGE}} package manager
RUN pip install uv
...
COPY . .
RUN python -m {{TEST_COMMAND}} tests/ --cov=src --cov-report=term-missing
...
# Stage 4: Optimized production with {{CLOUD_BASE_IMAGE}}
FROM gcr.io/{{CLOUD_BASE_IMAGE}}/python3-debian11 AS production
...
FROM golang:1.21-{{CLOUD_BASE_IMAGE}} AS base
...
FROM rust:1.70-{{CLOUD_BASE_IMAGE}} AS base
...
**{{LANGUAGE}} Spring Boot with GraalVM Native Image**
```dockerfile
# Pattern 5: {{LANGUAGE}} with GraalVM Native Image (sub-second startup)
...
Minimize {{CLOUD_CONTAINER_TOOL}} image size:

**Size Reduction Techniques**
```dockerfile
# {{CLOUD_BASE_IMAGE}}-based optimization
FROM {{CLOUD_BASE_IMAGE}}:3.18
...
FROM golang:1.21-{{CLOUD_BASE_IMAGE}} AS builder
WORKDIR /build
COPY . .
# Build static binary
...
def optimize_dockerfile_layers(dockerfile_content):
    """
    Optimize {{CLOUD_PATH_DOCKERFILE}} layers
    """
    optimizations = []
...
FROM node:18-{{CLOUD_BASE_IMAGE}} AS builder
WORKDIR /app
...
**Security-Hardened {{CLOUD_PATH_DOCKERFILE}}**
```dockerfile
# Use specific version and minimal base image
FROM {{CLOUD_BASE_IMAGE}}:3.18.4
...
```yaml
# {{CLOUD_PATH_CI}}/docker-security.yml
name: {{CLOUD_CONTAINER_TOOL}} Security Scan
  ...
  scan:
    ...
    steps:
      ...
      - name: Run Hadolint
        ...
        with:
          dockerfile: {{CLOUD_PATH_DOCKERFILE}}
          format: sarif
...
FROM eclipse-temurin:17-jre-{{CLOUD_BASE_IMAGE}}
...
# {{CLOUD_INGRESS}} optimization
FROM {{CLOUD_INGRESS}}:{{CLOUD_BASE_IMAGE}}
COPY {{CLOUD_INGRESS}}-optimized.conf /etc/{{CLOUD_INGRESS}}/{{CLOUD_INGRESS}}.conf
# Enable gzip, caching, and connection pooling
```

### 7. {{CLOUD_CONTAINER_TOOL}} Compose Optimization
...
services:
  ...
  redis:
    image: redis:7-{{CLOUD_BASE_IMAGE}}
  ...
  {{CLOUD_INGRESS}}:
    image: {{CLOUD_INGRESS}}:{{CLOUD_BASE_IMAGE}}
    volumes:
      - type: bind
        source: ./{{CLOUD_INGRESS}}.conf
        target: /etc/{{CLOUD_INGRESS}}/{{CLOUD_INGRESS}}.conf
        read_only: true
    depends_on:
      app:
...
echo "🏗️  Building optimized {{CLOUD_CONTAINER_TOOL}} image..."

# Enable BuildKit
export DOCKER_BUILDKIT=1

# Build with cache
{{CLOUD_BUILD_COMMAND}}x build \
  --platform "${PLATFORMS}" \
...
# Analyze image size
echo "📊 Image analysis:"
{{CLOUD_RUN_COMMAND}} --rm -v /var/run/docker.sock:/var/run/docker.sock \
...
def generate_dockerfile_checklist():
    """Generate {{CLOUD_PATH_DOCKERFILE}} best practices checklist"""
    checklist = """
## {{CLOUD_PATH_DOCKERFILE}} Best Practices Checklist

### Base Image
- [ ] Use specific version tags (not :latest)
- [ ] Use minimal base images ({{CLOUD_BASE_IMAGE}}, slim, {{CLOUD_BASE_IMAGE}})
...
1. **Analysis Report**: Current {{CLOUD_PATH_DOCKERFILE}} issues and optimization opportunities
2. **Optimized {{CLOUD_PATH_DOCKERFILE}}**: Rewritten {{CLOUD_PATH_DOCKERFILE}} with all optimizations
...
# 4. Generate {{CLOUD_ORCHESTRATOR}} manifests for optimized containers
/{{CLOUD_ORCHESTRATOR}}-manifest
...
class IntegratedContainerConfig:
    def __init__(self):
        ...
        self.k8s_config = self.load_k8s_config()          # From /{{CLOUD_ORCHESTRATOR}}-manifest
        self.test_config = self.load_test_config()         # From /test-harness
        
    def generate_optimized_dockerfile(self):
        """Generate {{CLOUD_PATH_DOCKERFILE}} optimized for the specific application"""
    ...
    def generate_fastapi_dockerfile(self, security_level, deployment_target):
        """Generate optimized {{BE_FRAMEWORK}} {{CLOUD_PATH_DOCKERFILE}}"""
        dockerfile_content = {
    ...
    def select_base_image(self, language, security_level):
        ...
        base_images = {
            'python': {
                'minimal': 'python:3.11-{{CLOUD_BASE_IMAGE}}',
                'standard': 'python:3.11-slim-bookworm',
                'secure': 'chainguard/python:latest-dev',
                '{{CLOUD_BASE_IMAGE}}': 'gcr.io/{{CLOUD_BASE_IMAGE}}/python3-debian12'
            }
        }
        
        if security_level == 'strict':
            return base_images[language]['{{CLOUD_BASE_IMAGE}}']
...
```dockerfile
# {{CLOUD_PATH_DOCKERFILE}}.api - Generated from /api-scaffold + /docker-optimize
# Multi-stage build optimized for {{BE_FRAMEWORK}} applications
...
# Install system dependencies for building {{LANGUAGE}} packages
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential \
    && rm -rf /var/lib/apt/lists/*

# Copy and install {{LANGUAGE}} dependencies
COPY requirements.txt .
...
RUN pip install --user {{TEST_COMMAND}} {{TEST_COMMAND}}-cov

# Run tests during build (from /test-harness integration)
RUN python -m {{TEST_COMMAND}} tests/ --cov=src --cov-report=json --cov-report=term
...
# Copy {{LANGUAGE}} packages from dependencies stage
...
# Configure health check (integrates with {{CLOUD_ORCHESTRATOR}} health checks)
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD python -c "import requests; requests.get('http://localhost:8000/health', timeout=5)"
...
```dockerfile
# {{CLOUD_PATH_DOCKERFILE}}.db - Generated for database migrations from /db-migrate
FROM postgres:15-{{CLOUD_BASE_IMAGE}} AS base
...
```dockerfile
# {{CLOUD_PATH_DOCKERFILE}}.frontend - Generated from /frontend-optimize + /docker-optimize
# Multi-stage build for {{FE_FRAMEWORK}}/{{FE_FRAMEWORK}} applications
FROM node:18-{{CLOUD_BASE_IMAGE}} AS base
...
FROM {{CLOUD_INGRESS}}:{{CLOUD_BASE_IMAGE}} AS production
...
# Copy built application
COPY --from=build --chown=www:www /app/dist /usr/share/{{CLOUD_INGRESS}}/html

# Copy security reports
COPY --from=security-scan /tmp/*-report.json /var/log/security/

# Copy optimized {{CLOUD_INGRESS}} configuration
COPY {{CLOUD_INGRESS}}.conf /etc/{{CLOUD_INGRESS}}/{{CLOUD_INGRESS}}.conf

# Configure proper file permissions
RUN chown -R www:www /usr/share/{{CLOUD_INGRESS}}/html
RUN chmod -R 755 /usr/share/{{CLOUD_INGRESS}}/html
...
# Use dumb-init for proper signal handling
ENTRYPOINT ["dumb-init", "--"]
CMD ["{{CLOUD_INGRESS}}", "-g", "daemon off;"]
```

**{{CLOUD_ORCHESTRATOR}} Container Integration**
```yaml
# {{CLOUD_ORCHESTRATOR}}-optimized-deployment.yaml - From /{{CLOUD_ORCHESTRATOR}}-manifest + /docker-optimize
apiVersion: v1
kind: ConfigMap
metadata:
...
```yaml
# {{CLOUD_PATH_CI}}/container-pipeline.yml
...
jobs:
  build-and-optimize:
    ...
    - name: Set up {{CLOUD_CONTAINER_TOOL}} Buildx
    ...
    - name: Build and push container images
      ...
      with:
        context: .
        file: {{CLOUD_PATH_DOCKERFILE}}.${{ matrix.service }}
        push: true
        tags: |
    ...
    - name: Container performance testing
      run: |
        ...
        {{CLOUD_RUN_COMMAND}} -d --name test-${{ matrix.service }} \
          ${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}/${{ matrix.service }}:${{ github.sha }}
  ...
  deploy-to-staging:
    ...
    - name: Deploy to staging
      run: |
        # Update {{CLOUD_ORCHESTRATOR}} manifests with new image tags
        # Apply optimized {{CLOUD_ORCHESTRATOR}} configurations
        kubectl set image {{CLOUD_PATH_DEPLOY}}/optimized-api \
...
class ContainerOptimizationMonitor:
    ...
    def monitor_optimization_metrics(self):
        ...
        for container in containers:
            ...
            # Update {{CLOUD_MONITORING_TOOL}} metrics
...
This comprehensive integration ensures containers are optimized across the entire development lifecycle, from build-time optimization through runtime monitoring and {{CLOUD_ORCHESTRATOR}} deployment.