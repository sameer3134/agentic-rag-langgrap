...
# {{CLOUD_ORCHESTRATOR}} Manifest Generation

You are a {{CLOUD_ORCHESTRATOR}} expert specializing in creating production-ready manifests, {{CLOUD_PACKAGE_MANAGER}} charts, and cloud-native deployment configurations. Generate secure, scalable, and maintainable {{CLOUD_ORCHESTRATOR}} resources following best practices and GitOps principles.

## Context
The user needs to create or optimize {{CLOUD_ORCHESTRATOR}} manifests for deploying applications. Focus on production readiness, security hardening, resource optimization, observability, and multi-environment configurations.
...
Analyze the application to determine {{CLOUD_ORCHESTRATOR}} requirements:
...
class AdvancedK8sAnalyzer:
    ...
    def _detect_app_type(self, app_path):
        ...
        indicators = {
            'web': ['{{CLOUD_INGRESS}}.conf', 'httpd.conf', 'index.html'],
    ...
    def _identify_services(self, app_path):
        ...
        # Check {{CLOUD_PATH_COMPOSE}}
        compose_file = Path(app_path) / '{{CLOUD_PATH_COMPOSE}}'
...
```yaml
apiVersion: networking.{{CLOUD_ORCHESTRATOR}}.io/v1
kind: Ingress
metadata:
  name: ${APP_NAME}
  namespace: ${NAMESPACE}
  annotations:
    ...
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/rate-limit: "100"
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/ssl-redirect: "true"
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/force-ssl-redirect: "true"
spec:
  ingressClassName: {{CLOUD_INGRESS}}
  tls:
  - hosts:
    - ${DOMAIN}
...
### 6. {{CLOUD_PACKAGE_MANAGER}} Chart Generation

Create production {{CLOUD_PACKAGE_MANAGER}} charts:
...
apiVersion: v2
name: $chart_name
description: A {{CLOUD_PACKAGE_MANAGER}} chart for $chart_name
type: application
version: 0.1.0
appVersion: "1.0.0"
keywords:
...
**FluxCD GitOps Setup**
```yaml
# {{CLOUD_PATH_IAC}}/flux-system/gotk-sync.yaml
...
spec:
  ...
  ref:
    branch: main
  url: https://github.com/org/{{CLOUD_ORCHESTRATOR}}-gitops
---
...
---
# Advanced Kustomization with {{CLOUD_PACKAGE_MANAGER}}
apiVersion: kustomize.config.{{CLOUD_ORCHESTRATOR}}.io/v1beta1
kind: Kustomization

resources:
  - namespace.yaml
  - ../../base
  - {{CLOUD_PATH_MONITORING}}/
  - security/
...
configMapGenerator:
  - name: app-config
    ...
    literals:
      ...
      - CACHE_TTL=3600
    files:
      - {{CLOUD_PATH_CONFIG}}/production.yaml
...
spec:
  ...
  - to:
    - namespaceSelector:
      ...
      podSelector:
        matchLabels:
          {{CLOUD_ORCHESTRATOR}}-app: kube-dns
    ports:
    - protocol: UDP
      port: 53
    - protocol: TCP
        ...
        properties:
          labels:
            ...
            items:
              type: string
  targets:
    - target: admission.{{CLOUD_ORCHESTRATOR}}.gatekeeper.sh
      rego: |
...
---
# Service Mesh Security ({{CLOUD_INGRESS}})
...
spec:
  ...
  - from:
    - source:
        principals: ["cluster.local/ns/{{CLOUD_PATH_MONITORING}}/sa/prometheus"]
    to:
    - operation:
        methods: ["GET"]
        paths: ["/metrics"]
...
**{{CLOUD_TRACING_TOOL}} Integration**
```yaml
# {{CLOUD_TRACING_TOOL}} Collector
  ...
  config: |
    ...
    processors:
      ...
      k8sattributes:
        ...
        extract:
          metadata:
            - {{CLOUD_ORCHESTRATOR}}.pod.name
            - {{CLOUD_ORCHESTRATOR}}.pod.uid
            - {{CLOUD_ORCHESTRATOR}}.deployment.name
            - {{CLOUD_ORCHESTRATOR}}.namespace.name
            - {{CLOUD_ORCHESTRATOR}}.node.name
            - {{CLOUD_ORCHESTRATOR}}.pod.start_time
...
spec:
  ...
  - port: metrics
    ...
    - sourceLabels: [__meta_kubernetes_pod_ip]
      ...
      replacement: $1:9090
---
# Custom {{CLOUD_MONITORING_TOOL}} Rules
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: ${APP_NAME}-rules
...
---
# {{CLOUD_MONITORING_TOOL}} Dashboard ConfigMap
apiVersion: v1
kind: ConfigMap
metadata:
...
**Multi-Cluster {{CLOUD_CD_TOOL}} Application**
```yaml
...
spec:
  ...
  - git:
      repoURL: https://github.com/org/{{CLOUD_ORCHESTRATOR}}-manifests
      revision: HEAD
      directories:
...
class ManifestValidator:
    ...
    def validate_manifest(self, manifest_file):
        """
        Validate {{CLOUD_ORCHESTRATOR}} manifest
        """
...
apiVersion: autoscaling.{{CLOUD_ORCHESTRATOR}}.io/v1
...
**{{CLOUD_CI_TOOL}} Workflow**
```yaml
# {{CLOUD_PATH_CI}}/deploy.yml
name: Deploy to {{CLOUD_ORCHESTRATOR}}

on:
  push:
    branches: [main]
    paths: ['src/**', '{{CLOUD_PATH_K8S}}/**', '{{CLOUD_PATH_DOCKERFILE}}']
  pull_request:
    branches: [main]
...
jobs:
  build-and-deploy:
    ...
    - name: Build and push {{CLOUD_CONTAINER_TOOL}} image
    ...
    - name: Install kubectl and kustomize
      run: |
        curl -LO "https://dl.{{CLOUD_ORCHESTRATOR}}.io/release/$(curl -L -s https://dl.{{CLOUD_ORCHESTRATOR}}.io/release/stable.txt)/bin/linux/amd64/kubectl"
        chmod +x kubectl && sudo mv kubectl /usr/local/bin/
        curl -s "https://raw.githubusercontent.com/kubernetes-sigs/kustomize/master/hack/install_kustomize.sh" | bash
        sudo mv kustomize /usr/local/bin/
    
    - name: Validate {{CLOUD_ORCHESTRATOR}} manifests
      run: |
        kubectl --dry-run=client --validate=true apply -k {{CLOUD_PATH_K8S}}/overlays/staging
    
    - name: Deploy to staging
      if: github.ref == 'refs/heads/main'
      run: |
        cd {{CLOUD_PATH_K8S}}/overlays/staging
        kustomize edit set image app=${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}:${{ steps.gitversion.outputs.semVer }}
        {{CLOUD_DEPLOY_COMMAND}} -k .
        kubectl rollout status {{CLOUD_PATH_DEPLOY}}/${APP_NAME} -n staging --timeout=300s
    
    - name: Run integration tests
      if: github.ref == 'refs/heads/main'
      run: |
        # Wait for deployment to be ready
        kubectl wait --for=condition=available --timeout=300s {{CLOUD_PATH_DEPLOY}}/${APP_NAME} -n staging
        # Run tests
    ...
    - name: Deploy to production
      ...
      run: |
        cd {{CLOUD_PATH_K8S}}/overlays/production
        kustomize edit set image app=${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}:${{ steps.gitversion.outputs.semVer }}
        {{CLOUD_DEPLOY_COMMAND}} -k .
        kubectl rollout status {{CLOUD_PATH_DEPLOY}}/${APP_NAME} -n production --timeout=600s
```
...
3. **GitOps Repository Structure**: Multi-environment with FluxCD/{{CLOUD_CD_TOOL}}
4. **Observability Stack**: {{CLOUD_TRACING_TOOL}}, {{CLOUD_MONITORING_TOOL}}, {{CLOUD_MONITORING_TOOL}}, {{CLOUD_TRACING_TOOL}}
...
8. **CI/CD Pipeline**: Complete {{CLOUD_CI_TOOL}} workflow with security scanning
...
**Enterprise {{CLOUD_ORCHESTRATOR}} Pipeline**
```bash
...
# 2. Optimize containers for {{CLOUD_ORCHESTRATOR}}
/docker-optimize
...
/security-scan
scan_types: ["{{CLOUD_ORCHESTRATOR}}", "container", "iac", "rbac"]
compliance: ["cis", "nsa", "pci"]

# 4. Generate production {{CLOUD_ORCHESTRATOR}} manifests
...
**Integrated {{CLOUD_ORCHESTRATOR}} Configuration**
```python
# {{CLOUD_ORCHESTRATOR}}-integration-config.py - Shared across all commands
class IntegratedKubernetesConfig:
    ...
    def generate_application_manifests(self):
        """Generate complete {{CLOUD_ORCHESTRATOR}} manifests for the application stack"""
        manifests = {
...
**Database Integration with {{CLOUD_ORCHESTRATOR}}**
```yaml
# database-{{CLOUD_ORCHESTRATOR}}-manifests.yaml - From /db-migrate + /{{CLOUD_ORCHESTRATOR}}-manifest
apiVersion: v1
kind: Secret
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: postgresql
        image: postgres:15-{{CLOUD_BASE_IMAGE}}
        ports:
        - name: postgresql
          containerPort: 5432
        env:
...
```yaml
# fullstack-{{CLOUD_ORCHESTRATOR}}-deployment.yaml - Integration across all commands
apiVersion: v1
kind: Namespace
metadata:
  name: fullstack-app
...
metadata:
  ...
  annotations:
    ...
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/force-ssl-redirect: "true"
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/rate-limit: "100"
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/rate-limit-window: "1m"
    cert-manager.io/cluster-issuer: "letsencrypt-prod"
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/add-base-url: "true"
    {{CLOUD_INGRESS}}.ingress.kubernetes.io/proxy-buffer-size: "8k"
spec:
  ingressClassName: {{CLOUD_INGRESS}}
  tls:
  - hosts:
...
```yaml
# security-{{CLOUD_ORCHESTRATOR}}-manifests.yaml - From /security-scan integration
apiVersion: v1
kind: ServiceAccount
metadata:
...
---
apiVersion: rbac.authorization.{{CLOUD_ORCHESTRATOR}}.io/v1
kind: Role
metadata:
  name: api-role
  namespace: fullstack-app
...
roleRef:
  kind: Role
  name: api-role
  apiGroup: rbac.authorization.{{CLOUD_ORCHESTRATOR}}.io
...
```yaml
# monitoring-{{CLOUD_ORCHESTRATOR}}-manifests.yaml - Complete observability stack
apiVersion: v1
kind: ServiceMonitor
metadata:
  name: api-monitor
...
---
# {{CLOUD_TRACING_TOOL}} tracing configuration
apiVersion: v1
kind: ConfigMap
metadata:
  name: jaeger-config
...
```yaml
# autoscaling-{{CLOUD_ORCHESTRATOR}}-manifests.yaml - Intelligent scaling based on multiple metrics
...
```yaml
# {{CLOUD_PATH_CI}}/{{CLOUD_ORCHESTRATOR}}-deployment.yml
name: {{CLOUD_ORCHESTRATOR}} Deployment Pipeline
...
jobs:
  deploy-to-kubernetes:
    ...
    - name: Setup {{CLOUD_PACKAGE_MANAGER}}
      uses: azure/setup-helm@v3
      with:
        version: 'v3.12.0'
    
    # 2. Authenticate with cluster
    - name: Configure {{CLOUD_PROVIDER}} credentials
    ...
    - name: Validate {{CLOUD_ORCHESTRATOR}} manifests
      run: |
        # Validate syntax
        kubectl --dry-run=client apply -f {{CLOUD_PATH_K8S}}/
        
        # Security validation with kubesec
        {{CLOUD_RUN_COMMAND}} --rm -v $(pwd):/workspace kubesec/kubesec:latest scan /workspace/{{CLOUD_PATH_K8S}}/*.yaml
        
        # Policy validation with OPA Gatekeeper
        conftest verify --policy opa-policies/ {{CLOUD_PATH_K8S}}/
    ...
    - name: Deploy to staging
      ...
      run: |
        # Update image tags
        sed -i "s|registry.company.com/api:.*|registry.company.com/api:${{ github.sha }}|g" {{CLOUD_PATH_K8S}}/api-deployment.yaml
        sed -i "s|registry.company.com/frontend:.*|registry.company.com/frontend:${{ github.sha }}|g" {{CLOUD_PATH_K8S}}/frontend-deployment.yaml
        
        # Apply manifests to staging namespace
        {{CLOUD_DEPLOY_COMMAND}} -f {{CLOUD_PATH_K8S}}/ --namespace=staging
        
        # Wait for rollout to complete
        kubectl rollout status {{CLOUD_PATH_DEPLOY}}/api-deployment --namespace=staging --timeout=300s
        kubectl rollout status {{CLOUD_PATH_DEPLOY}}/frontend-deployment --namespace=staging --timeout=300s
    ...
    - name: Run integration tests
      ...
      run: |
        ...
        # Get service URLs
        API_URL=$({{CLOUD_STATUS_COMMAND}} service api-service --namespace=staging -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
        
        # Run tests from /test-harness
        {{TEST_COMMAND}} tests/integration/ --api-url="http://${API_URL}:8000" -v
    ...
    - name: Deploy to production
      ...
      run: |
        ...
        {{CLOUD_DEPLOY_COMMAND}} -f {{CLOUD_PATH_K8S}}/ --namespace=production
        
        # Monitor rollout
        kubectl rollout status {{CLOUD_PATH_DEPLOY}}/api-deployment --namespace=production --timeout=600s
        kubectl rollout status {{CLOUD_PATH_DEPLOY}}/frontend-deployment --namespace=production --timeout=600s
        
        # Verify deployment health
        {{CLOUD_STATUS_COMMAND}} pods --namespace=production -l app=api
        {{CLOUD_STATUS_COMMAND}} pods --namespace=production -l app=frontend
    ...
    - name: Post-deployment verification
      ...
      run: |
        # Health checks
        kubectl exec -n production {{CLOUD_PATH_DEPLOY}}/api-deployment -- curl -f http://localhost:8000/health
    ...
    - name: Rollback on failure
      ...
      run: |
        ...
        kubectl rollout undo {{CLOUD_PATH_DEPLOY}}/api-deployment --namespace=production
        kubectl rollout undo {{CLOUD_PATH_DEPLOY}}/frontend-deployment --namespace=production
        
        # Notify team
        echo "Deployment failed and rolled back" >> $GITHUB_STEP_SUMMARY
```

This comprehensive integration ensures that {{CLOUD_ORCHESTRATOR}} deployments leverage all optimizations from container builds, security hardening, database migrations, and monitoring configurations while providing enterprise-grade reliability and observability.
