...
complexity: advanced
priority: high
tags: [monitoring, cloud, infra, slo]
depends_on: []
chains_to: []
skip_if: [no_cloud]
version: 1.0.0
...
class MonitoringAssessment:
    ...
    def _detect_infrastructure(self, project_path):
        ...
        if (Path(project_path) / '{{CLOUD_PATH_COMPOSE}}').exists():
            infrastructure['orchestration'] = 'docker-compose'
        elif (Path(project_path) / '{{CLOUD_ORCHESTRATOR}}').exists():
...
### 2. {{CLOUD_MONITORING_TOOL}} Setup

Implement {{CLOUD_MONITORING_TOOL}}-based monitoring:

**{{CLOUD_MONITORING_TOOL}} Configuration**
```yaml
# prometheus.yml
global:
  scrape_interval: 15s
...
scrape_configs:
  # {{CLOUD_MONITORING_TOOL}} self-monitoring
  - job_name: 'prometheus'
    static_configs:
...
// Recording rules for {{CLOUD_MONITORING_TOOL}}
...
### 3. {{CLOUD_MONITORING_TOOL}} Dashboard Setup
...
Implement {{CLOUD_TRACING_TOOL}}-based tracing:

**{{CLOUD_TRACING_TOOL}} Configuration**
```typescript
// tracing.ts
...
export class TracingSetup {
    ...
    constructor(serviceName: string, environment: string) {
        ...
        const prometheusExporter = new PrometheusExporter({
            ...
            console.log('{{CLOUD_MONITORING_TOOL}} metrics server started on port 9464');
        });
...
# Add {{CLOUD_ORCHESTRATOR}} metadata
<filter kubernetes.**>
...
class StructuredLogger:
    ...
    def _get_trace_context(self) -> Optional[Dict[str, str]]:
        """Extract trace context from {{CLOUD_TRACING_TOOL}}"""
...
from flask import {{BE_FRAMEWORK}}, request, g
import uuid

def setup_request_logging(app: {{BE_FRAMEWORK}}, logger: StructuredLogger):
    @app.before_request
    def before_request():
...
route:
  ...
  routes:
    # Critical alerts go to {{CLOUD_ALERTING_TOOL}}
    - match:
        severity: critical
      receiver: pagerduty
      continue: true
...
Deploy monitoring stack with {{CLOUD_IAC_TOOL}}:

**{{CLOUD_IAC_TOOL}} Configuration**
```hcl
# monitoring.tf
module "prometheus" {
...
module "grafana" {
  ...
  datasources = [
    {
      name = "{{CLOUD_MONITORING_TOOL}}"
      type = "prometheus"
      url  = "http://prometheus:9090"
    },
    {
      name = "{{CLOUD_LOG_STACK}}"
      type = "loki"
      url  = "http://loki:3100"
    },
    {
      name = "{{CLOUD_TRACING_TOOL}}"
      type = "jaeger"
...
5. **Dashboard Templates**: Ready-to-use {{CLOUD_MONITORING_TOOL}} dashboards
