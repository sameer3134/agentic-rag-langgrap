...
complexity: advanced
priority: critical
tags: ["accessibility", "frontend", "testing", "components"]
depends_on: []
chains_to: []
skip_if: ["no_frontend"]
version: 1.0.0
...
const { AxePuppeteer } = require('{{FE_A11Y_TOOL}}');
const puppeteer = require('puppeteer');
const pa11y = require('{{FE_A11Y_TOOL}}');
...
import { render } from '{{FE_TEST_FRAMEWORK}}';
import { axe, toHaveNoViolations } from '{{FE_TEST_FRAMEWORK}}-{{FE_A11Y_TOOL}}';
...
// {{FE_FRAMEWORK}} accessibility components
import {{FE_FRAMEWORK}} from '{{FE_FRAMEWORK}}';
...
jobs:
  a11y-tests:
    ...
    - name: Setup {{FE_RUNTIME}}
      uses: actions/setup-node@v3
      with:
        {{FE_RUNTIME}}-version: '18'
    ...
    - name: Start server
      run: |
        {{FE_START_COMMAND}} &
        {{FE_RUNNER}} wait-on http://localhost:3000
    
    - name: Run {{FE_A11Y_TOOL}} accessibility tests
      run: {{FE_RUNNER}} test:a11y
    
    - name: Run {{FE_A11Y_TOOL}} tests
      run: |
        {{FE_RUNNER}} {{FE_A11Y_TOOL}} http://localhost:3000 \
          --reporter cli \
          --standard WCAG2AA \
          --threshold 0
    
    - name: Run {{FE_A11Y_TOOL}} CI
      run: |
        npm install -g @{{FE_A11Y_TOOL}}/cli
        {{FE_A11Y_TOOL}} autorun --config={{FE_A11Y_TOOL}}rc.json
...
if [ -n "$CHANGED_FILES" ]; then
    ...
    {{FE_RUNNER}} test:a11y -- $CHANGED_FILES
